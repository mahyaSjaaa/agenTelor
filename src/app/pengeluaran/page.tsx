...
import { poppins_tebal, poppins_normal } from '@/component/font';
...
export default function PemasukanPage() {
    ...
    const handleInput = async(kg:number, price:number, tgl:string) => {
      const date = new Date()
      ...
      const {error} = await supabase.from('kuitansi').insert({created_at:tgl, jumlah:kg, harga:price, tanggal:formatted, jenis:"pengeluaran"})
      if(error){
        console.log(error);
      }else{
    ...
    useEffect(() => {
      const fetch = async () => {
        const {data, error} = await supabase.from('kuitansi').select().order('id', {ascending:false}).eq('jenis', "pengeluaran")
        setDatas(data)
        setIsLoading(false)
        if(error){
          console.log(error);
    ...
    return (
        <div className="pt-25 pl-20 h-screen">
            <main className="block">
                <h1 className={`${poppins_tebal.className} text-[20px] lg:text-[24px] px-5 lg:px-20 pt-6`}>Catat pengeluaran</h1>
                <div className='lg:flex lg:justify-between justify-between lg:items-center items-center px-5 pb-10 lg:px-20'>
                    ...
                    <button  onClick={() =>handleInput(berat, harga, sekarang.toLocaleDateString())} className='mt-2 px-1.5 bg-black px-6 mr-5 text-white py-1.5 rounded-full text-[12px] ml-20'>Simpan</button>
                </div>
                          ...
                          <div className="pb-4 mr-5">
                                ...
                                {isLoading ? <p>loading..</p> : (
                                    datas&&datas.map((data: Kuitansi) => (
                                        <div key={data.id} className="lg:ml-20 lg:mr-25 lg:px-20 px-3 mr-6 flex justify-between items-center border-2 border-black rounded-xl py-1.5 bg-red-200 mb-2">
